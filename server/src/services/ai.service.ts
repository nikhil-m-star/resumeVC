export class AIService {
    ...
    public async generateSampleResume(category: string, companyType?: string, existingContext?: string): Promise<Record<string, unknown>> {
        ...
        const systemPrompt = [
            ...
            '          "description": "- GPA or highlights", "aiGenerated": true }',
            '    ]}',
            '  ]',
            '}',
            'CRITICAL RULES:',
            '- EVERY field must have a realistic value. NEVER leave any field as empty string "", "...", or placeholder.',
            '- For EVERY item you generate or invent (not from user data), set "aiGenerated": true.',
            '- If you are building on the user\'s existing data (their real name, company, role), set "aiGenerated": false for those items.',
